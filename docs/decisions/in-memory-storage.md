...
## Decision
Utilize an in-memory Python dictionary (`_tasks: dict[int, dict]`) in `main.py` paired with `itertools.count(1)` for task ID allocation.

Provide a `reset_store()` helper function to reset stored state between unit tests.

## Trade-offs & Consequences
* **Benefits**: No database drivers or external setup required; instant test execution (< 0.3s for 43 unit tests); lightweight Docker build.
